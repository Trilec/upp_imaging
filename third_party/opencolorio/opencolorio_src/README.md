...
process exits after LUT use. Generate or verify it with
`python tools/generate_ocio_file_transform.py [--check]` from the repository root.
The original upstream source remains unchanged; the manifest compiles the
overlay instead of the original translation unit. The preview regression
exercises file/environment configurations, LUTs, invalid LUTs and clean exit.
...
Generated headers: `third_party/opencolorio/OpenColorIO/OpenColorABI.h`, `third_party/opencolorio/OpenColorIO/OpenColorIOConfig.h`, `upstream/src/OpenColorIO/CPUInfoConfig.h`.

OpenColorIO private quoted headers use the package-local `-iquote upstream/src/OpenColorIO` search path. This prevents generic private names such as `Logging.h` from colliding with dependency headers in U++'s aggregate include graph; pinned upstream OpenColorIO sources remain unmodified.

Validated toolchain: `CLANGx64` (`clang++ 21.1.1`, target `x86_64-w64-windows-gnu`).

Known warnings: case-mismatch `third_party/opencolorio/OpenColorIO/...` include-path warnings on Windows, plus the usual upstream chatter.
