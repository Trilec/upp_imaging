...
static void Classify(ImageSpec& spec)
{
	...
	if(spec.channels == 2 && IsAlphaName(spec.channel_names[1])) { spec.channel_layout = ChannelLayout::GrayAlpha; spec.alpha_channel = 1; return; }
	if(spec.channels == 3 && IsName(spec.channel_names[0], "r") && IsName(spec.channel_names[1], "g") && IsName(spec.channel_names[2], "b")) { spec.channel_layout = ChannelLayout::RGB; return; }
	if(spec.channels == 4 && IsName(spec.channel_names[0], "r") && IsName(spec.channel_names[1], "g") && IsName(spec.channel_names[2], "b") && IsAlphaName(spec.channel_names[3])) { spec.channel_layout = ChannelLayout::RGBA; spec.alpha_channel = 3; return; }
	if(spec.channels == 1) { spec.channel_layout = ChannelLayout::Gray; return; }
...
static bool AddAttribute(const ParamValue& attribute, Metadata& metadata, Diagnostics* diagnostics)
{
	...
	if(type == TypeDesc::INT64) { metadata.Set(key, Value(attribute.get<int64_t>())); return true; }
	if(type == TypeDesc::FLOAT || type == TypeDesc::DOUBLE) { metadata.Set(key, Value(attribute.get<double>())); return true; }
	if(type.arraylen > 0 && (type.basetype == TypeDesc::FLOAT || type.basetype == TypeDesc::DOUBLE || type.basetype == TypeDesc::INT)) {
		ValueArray array;
		for(int i = 0; i < type.arraylen; i++) array.Add(attribute.get<double>(i));
		metadata.Set(key, Value(array));
		return true;
	}
	if(diagnostics) diagnostics->Warning("unsupported metadata omitted", key);
	return false;
}
...
static bool ToBackendAttribute(const String& key, const Value& value, OIIO::ImageSpec& target, Diagnostics* diagnostics)
{
	if(value.Is<String>()) target.attribute(key.Begin(), value.To<String>().Begin());
	...
	else if(value.Is<int64>()) { int64_t v = value.To<int64>(); target.attribute(key.Begin(), TypeDesc::INT64, &v); }
		...
		for(const Value& item : array) { if(!item.Is<double>() && !item.Is<int>() && !item.Is<int64>()) return false; numbers.Add(item.Is<double>() ? item.To<double>() : item.Is<int>() ? item.To<int>() : (double)item.To<int64>()); }
		if(!numbers.IsEmpty()) target.attribute(key.Begin(), TypeDesc(TypeDesc::DOUBLE, (int)numbers.GetCount()), numbers.Begin());
	}
	else { if(diagnostics) diagnostics->Warning("unsupported metadata omitted", key); return false; }
	return true;
}
...
Result LoadImageFile(const String& path, ImageData& output, Diagnostics* diagnostics)
{
	...
	ImageBuf backend;
	std::string error;
	if(!UppImaging::LoadImage(path.Begin(), backend, &error)) return Fail(ResultCode::IOError, diagnostics, error.c_str(), path, "IMGIO_OPEN");
	const OIIO::ImageSpec& source = backend.spec();
	if(source.depth != 1 || backend.deep()) return Fail(ResultCode::Unsupported, diagnostics, "deep or volume image is unsupported", path, "IMGIO_SPEC");
	SampleType sample;
	if(!TypeToCore(source.format, sample)) return Fail(ResultCode::Unsupported, diagnostics, "unsupported or mixed sample format", path, "IMGIO_SAMPLE");
	...
	ROI roi(source.x, source.x + source.width, source.y, source.y + source.height, 0, 1, 0, source.nchannels);
	if(!backend.get_pixels(roi, source.format, candidate.buffer.Begin(), AutoStride)) return Fail(ResultCode::IOError, diagnostics, backend.geterror().c_str(), path, "IMGIO_PIXELS");
...
Result SaveImageFile(const String& path, const ImageData& image, Diagnostics* diagnostics)
{
	...
	for(const String& name : image.spec.channel_names) target.channelnames.emplace_back(name.Begin());
	...
	OIIO::ImageBuf backend(target, const_cast<byte*>(image.buffer.Begin()));
	UppImaging::InitializeOpenImageIO();
	std::string error;
	if(!UppImaging::SaveImage(path.Begin(), backend, &error)) return Fail(ResultCode::IOError, diagnostics, error.c_str(), path, "IMGIO_WRITE");
	return Result::Success();
}
