                     ...
                     Diagnostics* diagnostics)
{
	...
	String extension = IOFormatPolicy::Extension(path);
	if(!IOFormatPolicy::IsSupportedExtension(extension))
		return Fail(ResultCode::Unsupported, diagnostics,
	...
	if(IOFormatPolicy::RequiresZeroOrigin(extension) &&
	   (source.x != 0 || source.y != 0))
		return Fail(ResultCode::Unsupported, diagnostics,
		            String(IOFormatPolicy::FormatName(extension)) +
		            " data-window origin must be zero",
		            path, "IMGIO_SPEC");
	...
	Result policy = IOFormatPolicy::ValidateLoaded(extension, candidate.spec,
	                                               diagnostics, path);
	if(!policy)
		return policy;
                     ...
                     Diagnostics* diagnostics)
{
	...
	Result policy = IOFormatPolicy::ValidateSave(extension, image,
	                                            diagnostics, path);
	if(!policy)
		return policy;
	...
	IOFormatPolicy::ConfigureWriteTarget(extension, image, target);
