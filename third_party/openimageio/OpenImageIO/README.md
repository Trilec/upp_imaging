...
For Windows/MinGW builds, initialization registers a shutdown callback that
clears the main-thread input/output error tables and global error string before
U++ performs its static heap audit. Three local translation-unit wrappers expose
this cleanup without changing pinned upstream files. This fixes Debug GUI
