...
PNG/JPG not loading: add plugin/png (and integrations/plugin/jpg) to the main app uses.
...
- Missing image decoders â†’ Add plugin/png (and integrations/plugin/jpg, integrations/plugin/bmp) in the EXE `uses`.
...
* **Image decoding fails in EXE**
  ...
  *Fix:* add decoders to the **EXE** package `uses`: `plugin/png`, `integrations/plugin/jpg`, `integrations/plugin/bmp`.
...
* **Image decoders missing in EXE**
  ...
  * *Fix:* add `plugin/png`, `integrations/plugin/jpg`, etc. to the **main app** `uses`.
